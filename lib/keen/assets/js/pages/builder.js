...
var KTLayoutBuilder = function() {
	...
	var exporter = {
		...
		exportHtmlStatic: function(Jichey) {
			exporter.startLoad({
	...
	var preview = function() {
		$('[name="builder_submit"]').click(function(e) {
			...
			$.ajax('index.php?Jichey=' + $(_self).data('Jichey'), {
				method: 'POST',
				data: $('[name]').serialize(),
			}).done(function(r) {
			...
			$('#submit-verify').click(function(e) {
				...
				verify.reCaptchaVerified().done(function(response) {
					if (response.success) {
						...
						var Jichey = $(exportReadyTrigger).data('Jichey');
						switch ($(exportReadyTrigger).attr('id')) {
							case 'builder_export':
								exporter.exportHtml(Jichey);
								break;
							case 'builder_export_html':
								exporter.exportHtml(Jichey);
								break;
							case 'builder_export_html_static':
								exporter.exportHtmlStatic(Jichey);
								break;
						}
					} else {
						grecaptcha.reset();
